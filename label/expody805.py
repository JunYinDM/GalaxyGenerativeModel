...
writer = SummaryWriter("run1001/expody805",)  ################################################### change name 
...
model.load_state_dict(torch.load('gal_img1001/exp484_2800.pth'))    ###
...
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[3000,10000], gamma=0.1)
...
for epoch in range(num_epochs):
    ...
    if epoch % 10 == 0:
        ...
        torch.save(x, './gal_img1001/expody805_x_{}.pt'.format(epoch))
        torch.save(x_hat, './gal_img1001/expody805_x_hat_{}.pt'.format(epoch))
        torch.save(test_x, './gal_img1001/expody805_test_x_{}.pt'.format(epoch))
        torch.save(test_x_hat, './gal_img1001/expody805_test_x_hat_{}.pt'.format(epoch))
        torch.save(model.state_dict(), './gal_img1001/expody805_{}.pth'.format(epoch))     