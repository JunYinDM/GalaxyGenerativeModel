...
writer = SummaryWriter("run1001/exp441",)  ################################################### change name 
...
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[500,2000], gamma=0.1)
...
for epoch in range(num_epochs):
    ...
    ################################################## Loss function with regularizing Z ########################
        ...
        loss= criterion_mean(output, img) + criterion_mean(z[:,:7], label)/weight

        loss_recon= criterion_mean(output, img)
        loss_latent=criterion_mean(z[:,:7], label) /weight
    ...
    for data in test_dataloader:
       ...
       # test_loss = criterion(test_output, test_img) + criterion(z[:,:7], label) /(1e)  #  + 1e-5*  criterion(test_z[:,:7], test_label) 
        test_loss=  criterion_mean(test_output, test_img) + criterion_mean(test_z[:,:7], test_label)  /weight
        
        test_loss_recon=  criterion_mean(test_output, test_img)
        test_loss_latent= (criterion_mean(test_z[:,:7], test_label)  /weight)        
    ...
    if epoch % 10 == 0:
        ...
        torch.save(x, './gal_img1001/exp441_x_{}.pt'.format(epoch))
        torch.save(x_hat, './gal_img1001/exp441_x_hat_{}.pt'.format(epoch))
        torch.save(test_x, './gal_img1001/exp441_test_x_{}.pt'.format(epoch))
        torch.save(test_x_hat, './gal_img1001/exp441_test_x_hat_{}.pt'.format(epoch))
        torch.save(model.state_dict(), './gal_img1001/exp441_{}.pth'.format(epoch))     