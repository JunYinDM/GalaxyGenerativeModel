...
writer = SummaryWriter("run1001/exp413",)  ################################################### change name 
...
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[200,500,2000,4000], gamma=0.1)
...
for epoch in range(num_epochs):
    ...
    if epoch % 10 == 0:
        ...
        torch.save(x, './gal_img1001/exp413_x_{}.pt'.format(epoch))
        torch.save(x_hat, './gal_img1001/exp413_x_hat_{}.pt'.format(epoch))
        torch.save(test_x, './gal_img1001/exp413_test_x_{}.pt'.format(epoch))
        torch.save(test_x_hat, './gal_img1001/exp413_test_x_hat_{}.pt'.format(epoch))
        torch.save(model.state_dict(), './gal_img1001/exp413_{}.pth'.format(epoch))     