...
from model import autoencoder_999
...
writer = SummaryWriter("run1001/exp207",)  ################################################### change name 
...
for epoch in range(num_epochs):
    ...
    ################################################## Loss function with regularizing Z ########################
        loss = criterion(output, img)   # + 1e-7*torch.norm(z,p=1)
    ...
    if epoch % 10 == 0:
        ...
        torch.save(x, './gal_img1001/exp207_x_{}.pt'.format(epoch))
        torch.save(x_hat, './gal_img1001/exp207_x_hat_{}.pt'.format(epoch))
        torch.save(test_x, './gal_img1001/exp207_test_x_{}.pt'.format(epoch))
        torch.save(test_x_hat, './gal_img1001/exp207_test_x_hat_{}.pt'.format(epoch))
        torch.save(model.state_dict(), './gal_img1001/exp207_{}.pth'.format(epoch))       
