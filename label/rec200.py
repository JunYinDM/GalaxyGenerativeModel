...
dataset= trainlabelDataset_reduced()
dataloader= DataLoader(dataset=dataset, batch_size=64,shuffle=True,drop_last=True,num_workers=8)

test_dataset = testlabelDataset_reduced()
test_dataloader= DataLoader(dataset=test_dataset, batch_size=64,shuffle=True,drop_last=True,num_workers=8)




writer = SummaryWriter("run1001/rec200",)  ################################################### change name 
...
for epoch in range(num_epochs):
    ...
    ################################################## Loss function with regularizing Z ########################
        ...
        loss=  (criterion_none(output, img).sum(dim=3).sum(dim=2).sum(dim=1)* 60/ flux_for_scaling).mean() +  ( criterion_none(z[:,:5], label).sum(dim=1)*1/6* flux_for_scaling).mean()  +  reg*torch.norm(z[:,5:],p=1)
    ...
    for data in test_dataloader:
       ...
       # test_loss = criterion(test_output, test_img) + criterion(z[:,:7], label) /(1e)  #  + 1e-5*  criterion(test_z[:,:7], test_label) 
        test_loss=  (criterion_none(test_output, test_img).sum(dim=3).sum(dim=2).sum(dim=1)* 60/ test_flux_for_scaling).mean() +  ( criterion_none(test_z[:,:5], test_label).sum(dim=1)*1/6* test_flux_for_scaling).mean()   +  reg*torch.norm(test_z[:,5:],p=1)
    ...
    if epoch % 10 == 0:
        ...
        torch.save(x, './gal_img1001/rec200_x_{}.pt'.format(epoch))
        torch.save(x_hat, './gal_img1001/rec200_x_hat_{}.pt'.format(epoch))
        torch.save(test_x, './gal_img1001/rec200_test_x_{}.pt'.format(epoch))
        torch.save(test_x_hat, './gal_img1001/rec200_test_x_hat_{}.pt'.format(epoch))
        torch.save(model.state_dict(), './gal_img1001/rec200_{}.pth'.format(epoch))     
