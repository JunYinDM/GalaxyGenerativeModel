...
writer = SummaryWriter("run1001/expody801",)  ################################################### change name 
...
model.load_state_dict(torch.load('gal_img1001/exp479_9710.pth'))    ###
...
scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[1500,10000], gamma=0.1)
...
for epoch in range(num_epochs):
    ...
    ################################################## Loss function with regularizing Z ########################
        ...
        loss= (( criterion_none(output, img)/(img.sum(dim=3).sum(dim=2).sum(dim=1))).mean()) + (criterion_none(z[:,:7], label)).mean() /weight
    ...
    if epoch % 10 == 0:
        ...
        torch.save(x, './gal_img1001/expody801_x_{}.pt'.format(epoch))
        torch.save(x_hat, './gal_img1001/expody801_x_hat_{}.pt'.format(epoch))
        torch.save(test_x, './gal_img1001/expody801_test_x_{}.pt'.format(epoch))
        torch.save(test_x_hat, './gal_img1001/expody801_test_x_hat_{}.pt'.format(epoch))
        torch.save(model.state_dict(), './gal_img1001/expody801_{}.pth'.format(epoch))     